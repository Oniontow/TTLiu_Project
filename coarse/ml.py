...
def plot_accuracy_comparison(accuracies, model_names):
    ...
    plt.title('Comparison of Coarse/Fine-Grained Results', fontsize=16)
...
def main():
    ...
    accuracies = []
    model_names = ['Baseline', 'Coarse-Grained', 'Ideal']
    ...
    quantized_model_lightning_noise = add_mac_noise_hook_to_quantized_model(
        quantized_model_lightning_noise, noise_mean=6, noise_std=1
    )
    print("\n=== Evaluating Baseline Model ===")
    acc = evaluate(quantized_model_lightning_noise, torch.device("cpu"), test_loader, label="Baseline")
    ...
    quantized_model_smalltriangle_noise = add_mac_noise_hook_to_quantized_model(
        quantized_model_smalltriangle_noise, noise_mean=9, noise_std=1
    )
    print("\n=== Evaluating Coarse-Grained Model ===")
    acc = evaluate(quantized_model_smalltriangle_noise, torch.device("cpu"), test_loader, label="Coarse-Grained")
    accuracies.append(acc)
    ...
    quantized_model_bigtriangle_noise = add_mac_noise_hook_to_quantized_model(
        quantized_model_bigtriangle_noise, noise_mean=0, noise_std=0
    )
    print("\n=== Evaluating Ideal Model ===")
    acc = evaluate(quantized_model_bigtriangle_noise, torch.device("cpu"), test_loader, label="Ideal")
    accuracies.append(acc)
